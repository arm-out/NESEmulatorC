...
{
public:
	Demo_olc2C02() { sAppName = "nes2C02 Demonstration"; }
...
private: 
	...
	bool OnUserUpdate(float fElapsedTime)
	{
		...
		nes.controller[0] |= GetKey(olc::Key::Z).bHeld ? 0x80 : 0x00;     // A Button
		nes.controller[0] |= GetKey(olc::Key::X).bHeld ? 0x40 : 0x00;     // B Button
		...
		for (int i = 0; i < 26; i++)
		{
			std::string s = hex(i, 2) + ": (" + std::to_string(nes.ppu.OAMpointer[i * 4 + 3])
				+ ", " + std::to_string(nes.ppu.OAMpointer[i * 4 + 0]) + ") "
				+ "ID: " + hex(nes.ppu.OAMpointer[i * 4 + 1], 2) +
				+" AT: " + hex(nes.ppu.OAMpointer[i * 4 + 2], 2);
